...
class TodoControllerIntegrationTest {
    ...
    fun `should add a todo successfully`() {
        ...
        // then
        todoRepository.findById(createdToDo!!.id).orElseThrow {
    ...
    fun `should return a bad request when given a faulty todo`() {
        val applicationError =
            webTestClient
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(
                    objectMapper.writeValueAsString("{\"title\": \"\", \"description\": \"todo\"}")
                )
                .exchange()
                .expectStatus()
                .isBadRequest
        ...
        assertThat(applicationError!!.message)
            .contains(
                "JSON parse error: Cannot construct instance of `de.claudioaltamura.kotlin_spring_boot_todo.dto.NewTodo` (although at least one Creator exists): no String-argument constructor/factory method to deserialize from String value"
            )
    }
    ...
    fun `should return todos find by title`() {
        ...
        val todoList =
            webTestClient
                .get()
                .uri { uriBuilder ->
                    uriBuilder.path("/todos").queryParam("title", todo.title).build()
                }
                .exchange()
                .expectStatus()
                .isOk
    ...
    fun `should update a todo when found`() {
        ...
        // then
        val todoFromDb =
            todoRepository.findById(changedTodo!!.id).orElseThrow {
                TodoNotFoundException("Todo not found")
            }
