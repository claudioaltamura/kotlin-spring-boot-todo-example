...
class CommentControllerIntegrationTest {
    ...
    fun `should return a bad request when given a faulty comment`() {
        ...
        assertThat(applicationError!!.message)
            .contains(
                "JSON parse error: Cannot construct instance of `de.claudioaltamura.kotlin_spring_boot_todo.dto.NewComment` (although at least one Creator exists): no String-argument constructor/factory method to deserialize from String value"
            )
    }
    ...
    fun `should return comments find by todo id`() {
        ...
        val comments =
            webTestClient
                .get()
                .uri { uriBuilder ->
                    uriBuilder.path("/comments").queryParam("todoId", todo.id).build()
                }
                .exchange()
                .expectStatus()
                .isOk
